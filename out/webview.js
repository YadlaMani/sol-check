...
function formatTransactionDetails(sig, tx) {
    const blockTime = tx.blockTime
        ? new Date(tx.blockTime * 1000).toLocaleString()
        : "N/A";
    const status = tx.meta?.err ? "Failed ❌" : "Success ✅";
    const fee = tx.meta?.fee ?? 0;
    const accounts = tx.transaction.message.accountKeys.map((k) => k.pubkey || k.toBase58());
...
${JSON.stringify(ix, null, 2)}</pre>
  `)
    ...
    <section>
      ...
      <ul>
        ${accounts
        .map((acc) => `<li><code>${acc}</code> - <a href="https://explorer.solana.com/address/${acc}?cluster=devnet" target="_blank">View ↗</a></li>`)
        .join("")}
      </ul>
    </section>
...
function formatTokenDetails(mint, data) {
...
function getWebviewContent(id, data) {
